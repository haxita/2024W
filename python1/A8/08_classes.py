...
# CapWords format. Instances of classes should use lower_case format, i.e., 
# analogous to function and variable names.
...
# construction in Python is actually done in two parts: First, a new instance 
# ofbthe class is created in the special method "__new__". Second, the 
# "__init__" method is called to initialize this new instance appropriately):
...
# Note: it is possible to assign values to attribues that do not even exist in
# the class definition. This is possible because Python is a dynamic language
# and allows for such flexibility. However, this is not recommended, as it can
# lead to confusion and errors. It is better to define all attributes in the
# class definition.

dog2.breed = "Golden Retriever"  # This will create a new attribute "breed"
...
# hierarchy of base classes and subclasses. In the example below, we will 
...
class Animal():
    ...
    def communicate(self):
        ...
        # should be implemented by concrete subclasses. Our "communicate" 
        # method is now called an abstract method, it does not have an (actual)
        # implementation. This can be indicated by raising a 
        # NotImplementedError. This way, we ensure that all subclasses of
        # "Animal" have to implement this method.
        raise NotImplementedError


# We can now create an instance of our base class "Animal"
...
# a class "SouthernLeopardFrog" that is a more specialized frog type:
...
# SouthernLeopardFrog     Canary
#
# Here are some examples of checking one type (you can provide multiple types 
# to check if any of them matches by providing a tuple of types):
...
print(issubclass(SouthernLeopardFrog, Frog))
print(issubclass(SouthernLeopardFrog, Cat))
...
class MyClass:
    ...
    # Just like class attributes, there can also be class methods, i.e., 
    # methods that do not belong to an instance/object but rather the class. 
    # Since there is no longer a dedicated instance/object, the "self" 
    # parameter is replaced with a "cls" parameter (again, the name is just a 
    # convention), which then contains the class instead of the object when the
    # method was invoked (this invocation can either be done through an object 
    # or through the class, but the class is the recommended approach,so it is 
    # clear for the reader that a class method rather than an object method is 
    # invoked). To mark a method as class method, a so-called decorator has to 
    # be used on top of the method definition: @classmethod
    # https://docs.python.org/3/library/functions.html#classmethod
    @classmethod
    def some_class_method(cls):
        print(f"this is a class method: {cls}")

    # There can also be static methods that take no object or class parameter 
    # at all. In this case, the object or the class at the invocation will 
    # simply be ignored entirely. To mark a method as static method, a 
    # so-called decorator has to be used on top of the method definition: 
    # @staticmethod
...
# "Overriding" the class attribute "var" via an instance/object actually 
# creates a new object attribute for this instance/object with the same name:
instance2.var = 4
...
class DerivedClass(MyClass):
    ...
    def some_method(self):
        ...
        # If we want to include the behavior of the base class method, we can 
        # do this again with "super" (uncomment to see the effect):
        # super().some_method()
...
class ProblematicDerivedClass(MyClass):
    ...
    def some_method(self):
        ...
        # the "some_method" during object initialization. Since we overrode 
        # this method here, this means that we actually execute this 
        # implementation. However, the object initialization of 
        # "ProblematicDerivedClass" is not yet finished (we are still in the 
        # super-initialization), and the attribute "self.b" does not exist yet.
...
# How can we solve such problems? There are multiple ways, depending on what 
# you ultimately need (not all are always possible/make sense):
...
# Properties are a way to control access to attributes of an object. They allow
# you to define a method that is called when an attribute is accessed or set.
# This can be useful to ensure that an attribute is always in a certain state
# when accessed or set. Properties are defined using the "property" function
# (see https://docs.python.org/3/library/functions.html#property).

# Let's say we want to ensure that the attribute "name" of our class "Dog" is
# always a string. We can use a property to ensure this:
...
# Note that we use the attribute "_name" to indicate a private attribute that
# should not be accessed directly. Instead, we use the property "name" to 
# access the attribute "name".

# We can now create an instance of the class "Dog" and set the attribute 
# "name". Although this looks like we are accessing a variable, we are actually
# calling the getter and setter methods of the property "name":
dog = Dog(name="bello")
print(dog.name)
dog.name = "barky" # This will call the setter method

# This will raise an error:
# dog.name = 5

# We can also define a property that is read-only by omitting the setter
# function. This can be useful if we want to make sure that an attribute is not
# changed after it has been set (read-only property).
...
# Abstract classes are not meant to be instantiated or called directly but 
# rather serve as a blueprint for derived classes. They can be used to define 
# a common interface that derived classes have to implement. We need to import
# module 'abc' for this purpose. https://docs.python.org/3/library/abc.html

# We already saw an example of an abstract method in the class "Animal" above.
# We can also define an abstract class that contains abstract methods 
# explicitly. This has the advantage of making our code more error-proof, as 
# the class then cannot be instantiated directly any more but has to be 
# derived from and the abstract methods have to be implemented in the derived 
# class. For this we need to inherit from the class "ABC" and use the
...
# We can create a class "Dog" that inherits from the abstract class "Animal" 
...
# We can now create an instance of the class "Dog", since we implement the
# abstract method "communicate":
dog = Dog(name="bello")
