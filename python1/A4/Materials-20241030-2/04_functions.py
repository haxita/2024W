...
def apply_function_to_list(*funcs, lst):
    for func in funcs:
        lst = [func(item) for item in lst]
    return lst
...
# We can now use the outer function to apply the inner function to a list of
# strings:
strings = ["HOUSE", "Chair ", " garage", " car", "TAblE"] 
funcs = (lower, strip)

result = apply_function_to_list(*funcs, lst=strings)
...
# This still works, but some IDE's will warn you if you pass a non-integer.
# In Visual Studio Code, you can enable this by setting 
# "python.analysis.typeCheckingMode" to "basic" or "strict" in the settings.
add(1, 2.123)
...
# Left-click on the function "add" and hit F12 to go to the definition in 
# Visual Studio Code. You will see the docstring there. This is also useful for
# your own functions to remember what they do. Just pointing at the function
# will show you the docstring in a tooltip.
add(1, 2.123)


################################################################################
# Advanced topic: function decorators
################################################################################

# Decorators are a way to modify or extend the behavior of functions or methods.
# They are a special kind of function that takes another function and extends it
...
add = debug_decorator(add)

# Decorators can also take arguments:
def debug_decorator_with_args(prefix: str):
    def decorator(func):
        def wrapper(*args, **kwargs):
            print(f"{prefix}: Arguments: {args}, {kwargs}")
            result = func(*args, **kwargs)
            print(f"{prefix}: Return value: {result}")
            return result
...
@debug_decorator_with_args("DEBUG")
def add(a, b):
    return a + b
...
# This is equivalent to:
add = debug_decorator_with_args("DEBUG")(add)

# Decorators can be used to add functionality to functions, such as logging,
# timing, caching, etc. They can also be used to enforce certain conditions
# (e.g., type checking) or to modify the return value of a function.

# For example, here we ensure that add() only accepts integers as arguments:
def enforce_integers(func):
    def wrapper(*args, **kwargs):
