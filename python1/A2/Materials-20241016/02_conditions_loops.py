...
# We can verify the id of a object with the id() function:
id_a = id(a)
id_b = id(b)
...
for x in range(0, 10, 2):  # [0, 10) step 2 -> 0, 2, 4, 6, 8
    print(x)
for x in range(10, 0, -2):  # [10, 0) step -2 -> 10, 8, 6, 4, 2
    print(x)
...
# Another example: we divide numbers from -10 to 10 by 2 and print the result.
# If the division by 2 is not possible (division by zero), we skip the division
# and print an error message.
for i in range(-10, 11):
    if i == 0:
        print(f"Division by zero is not possible for i={i}")
        continue
    print(f"{i} / 2 = {i / 2}")
